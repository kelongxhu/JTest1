...
//       redis.set("address", "guangzhou");   
       ...
       */  
       ...
       for(int i=0;i<list2.size();i++){   
           System.out.println("mget到值"+list2.get(i));   
       }   
       ...
       //INCR key 将key中储存的数字值增一。   
        ...
        for(int i=0;i<list3.size();i++){   
            System.out.println("website的键中取值:"+list3.get(i));   
        }   
           
        //HGETALL key返回哈希表key中，所有的域和值。   
        Map<String,String> map2 = redis.hgetAll("hash");   
        for(Map.Entry entry: map2.entrySet()) {   
             System.out.print("取出hash中map的所有键值："+entry.getKey() + ":" + entry.getValue()+ "\t");   
        }   
